...
source /dev/stdin <<<$(curl -fsSL https://raw.githubusercontent.com/community-scripts/ProxmoxVE/main/misc/core.func)
...
function get_bitbucket_creds() {
  ...
  if [ -z "$OAUTH_CLIENT_ID" ]; then
    if ! OAUTH_CLIENT_ID=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Enter OAuth Client ID (mandatory)" 10 58 --title "Bitbucket Config" 3>&1 1>&2 2>&3); then exit_script; fi
  fi
  if [ -z "$OAUTH_CLIENT_ID" ]; then msg_error "OAuth Client ID is mandatory"; exit; fi

  if [ -z "$OAUTH_CLIENT_SECRET" ]; then
    if ! OAUTH_CLIENT_SECRET=$(whiptail --backtitle "Proxmox VE Helper Scripts" --passwordbox "Enter OAuth Client Secret (mandatory)" 10 58 --title "Bitbucket Config" 3>&1 1>&2 2>&3); then exit_script; fi
  fi
  ...
  if [ -z "$var_ssh_authorized_key" ]; then
    if ! var_ssh_authorized_key=$(whiptail --backtitle "Proxmox VE Helper Scripts" --inputbox "Enter SSH Public Key (optional)" 10 58 --title "SSH KEYS" 3>&1 1>&2 2>&3); then exit_script; fi
  fi
...
virt-customize -q -a "${FILE}" \
  ...
  --run-command "echo 'vm.swappiness = 0' > /etc/sysctl.d/99-swappiness.conf" \
...
# SSH Key Configuration using Cloud-Init
if [ -n "$var_ssh_authorized_key" ]; then
  cat <<EOF >"$TEMP_DIR/authorized_keys"
$var_ssh_authorized_key
EOF
  qm set $VMID -sshkeys "$TEMP_DIR/authorized_keys" >/dev/null
  qm set $VMID -ciuser root >/dev/null
fi

# Network Configuration using Cloud-Init
if [ "$var_net" = "static" ]; then
  qm set $VMID -ipconfig0 ip=$var_ip,gw=$var_gateway >/dev/null
  if [ -n "$var_dns" ]; then
    qm set $VMID -nameserver $var_dns >/dev/null
  fi
else
